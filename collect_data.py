...
def process_image(image_path):  
    ...
    if results.pose_landmarks:
        ...
        return height, hand_span, shoulder_width, thigh_length
    else:
...
with open(csv_file, mode='w', newline='') as file:
    ...
    # Tulis header kolom
    writer.writerow(['Nama', 'Tinggi Badan (pixel)', 'Lebar Tangan (pixel)', 'Lebar Bahu (pixel)', 'Panjang Paha (pixel)'])
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".jpg"):
            ...
            # Ekstrak nama dari file (tanpa ekstensi)
            name = os.path.splitext(filename)[0]
            
            # Proses gambar dan ambil pengukuran
            height, hand_span, shoulder_width, thigh_length = process_image(image_path)
            
            if height is not None:
                # Konversi hasil ke format desimal dengan koma
                height_str = f"{height:.f}".replace('.', ',')
                ...
                writer.writerow([name, height_str, hand_span_str, shoulder_width_str, thigh_length_str])
            else:
